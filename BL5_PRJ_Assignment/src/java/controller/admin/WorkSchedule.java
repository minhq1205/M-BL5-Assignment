...
public class WorkSchedule extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        TimeDBContext dbSlot = new TimeDBContext();
        ArrayList<Time> slots = dbSlot.all();
        SessionDBContext lecDb = new SessionDBContext();
        ArrayList<Session> sessions = lecDb.getSessions(lid);
        
        request.setAttribute("slots", slots);
        request.setAttribute("dates", dates);
        request.setAttribute("sessions", sessions);
        request.getRequestDispatcher("/view/Manager/timetable.jsp").forward(request, response);
